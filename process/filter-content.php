...
function r_filter_recipe_content( $content ) {
	...
	#if server turn off file_get_content then we should use the above twoo code line number  13-14
	#$recipe_html			=	file_get_contents( 'recipe-template.php', true );
	#

	$recipe_html			=	str_replace( "INGREDIENTS_PH", $recipe_data['ingridients'], $recipe_html );
	...
	$rating_count			=	$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM ". $wpdb->prefix . "recipe_ratings WHERE recipe_id=%d AND user_ip=%s",
