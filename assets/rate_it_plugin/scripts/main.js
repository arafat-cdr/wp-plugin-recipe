jQuery(function($) {
	...
	$(document).on('submit', '#recipeForm', function(e) {
		...
		$("#recipeStatus").html('<div class="alert alert-info">Please wait ! We are submitting your reaipe </div>');
		...
		$.post( recipe_obj.ajax_url, formObj, function (data) {
			...
			if(data.status === 2) {
				...
				$("#recipeStatus").html('<div class="alert alert-success">Recipe Submitted Successfully  ! Admin approve it </div>');

			} else {

		       $("#recipeStatus").html('<div class="alert alert-danger">Unable to submit recipe ! Please fill all the  fields </div>');
